...
const ProfessorLink = () => {
      ...
      const fetchProfessorData = async () => {
        try {
          const professorQuery = query(
            ...
            where("uid", "==", user.uid) // Matching the field inside the document
          );
          const querySnapshot = await getDocs(professorQuery);

          if (!querySnapshot.empty) {
            const professorData = querySnapshot.docs[0].data();
            const name = `${professorData.firstName} ${professorData.lastName}`;
            setFullName(name);

            setFormData((prevData) => ({
              ...prevData,
              uploader: name, // Auto-fill uploader
            }));
          } else {
            console.warn("No professor found for this user.");
          }
        } catch (error) {
    ...
    if (user) {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          // Query links where uploader matches professor's full name
        ...
        console.error("Error fetching professor links:", error);
      }
    }
  };
  ...
  const handleAddWebsite = async (e) => {
    ...
    try {
      const newWebsite = { ...formData, uploader: fullName }; // Ensure uploader is set
      const docRef = await addDoc(collection(db, "website"), newWebsite);
      ...
      setFormData({
        ...
        course: "",
        description: "",
        uploader: fullName, // Keep uploader name after reset
      });
